# Smart Element for Thunderbird

This subdirectory is a Thunderbird WebExtension variant of the Smart Element browser extension.

The toolbar button opens Element Web in a Thunderbird content tab. By default it opens
`https://matrix.helmholtz.cloud/`; use the extension settings page to change the target URL, for example to
`https://matrix.helmholtz.cloud/`.

The Element enhancement content scripts are copied from the browser extension and still run on the
configured Element hosts. To build an installable add-on for Thunderbird, zip the contents of this
directory, not the directory itself, and use the `.xpi` extension.
